...
glyco_bin = bwareaopen(glyco_bin,5);%Para 2
glyco_bin= imdilate(glyco_bin,sed);
glyco_bin = imdilate(glyco_bin,sel);
glyco_bin = imerode(glyco_bin,sel);
glyco_bin = imerode(glyco_bin,sed);
%add a border
...
for label = 1:GL_nums
    ...
    % figure;
    % imshow(glyco_bin);
    glyco_bin = imdilate(glyco_bin,sed2);
    % figure;
    % imshow(glyco_bin);
    ...
    if numEdges > 1
        ...
        GL_boundary = false(size(GL_boundary));
        GL_boundary(CC.PixelIdxList{largestEdgeIdx}) = true;
    end
    % figure
    % imshow(GL_boundary)
