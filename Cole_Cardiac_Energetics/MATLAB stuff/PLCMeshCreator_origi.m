...
% Order to run in is 'PLCMeshCreator_origi (this) -> postProcessing ->
% checkmesh -> checkmesh_boundary_V_final (optional to see Boundary nodes)
...
%%%% border
img_in_double = double(padarray(img, [50, 50], 'both'));
...
%***ColeD changes:
% some images have pixel values 1 higher or lower than shown here. Might
% need to manually adjust for each image

% Identify regions based on image color
%:
[mito_size, mito_bin] = identifyRegions(img_in_double, 85);
[myo_size, myo_bin] = identifyRegions(img_in_double, 154);
[glyco_size, glyco_bin] = identifyRegions(img_in_double, 189);
...
% [tt_size, tt_bin] = identifyRegions(img_in_double, 163);
...
% This section is for images with only 1 color, and is meant for testing
...
% Detects all edges within the image
cellSL_binary = edge(cell_img,'log','thinning');
% figure
% imshow(cellSL_binary)

CC = bwconncomp(cellSL_binary);
...
% If multiple edges are found, keep only the largest one
    if numEdges > 1
        ...
        cellSL_binary = false(size(cellSL_binary));
        cellSL_binary(CC.PixelIdxList{largestEdgeIdx}) = true;
    end

% figure
% imshow(cellSL_binary)

% %***ColeD_long changes start:
% cellSL_binary = edge(cell_img,'log','thinning');
% % figure
% % imshow(cellSL_binary)
% 
% % Fills all the edges,leaving only the borders of our image
% cellSL_binary_filled = imfill(cellSL_binary,'holes');
% % imshow(cellSL_binary)
% 
% % Some invaginations are seen in the image, this widens them to better
% % define the border of our image and then again delete any
% % floating/straggling edges
% se =  strel('disk',4);
% cellSL_binary_eroded = imerode(cellSL_binary_filled,se);
% % figure
% % imshow(cellSL_binary)
% 
% cellSL_binary_cleaned = bwareaopen(cellSL_binary_eroded,5000);
% % figure
% % imshow(cellSL_binary)
% 
% % The entire image was filled in previously, now again find only its edges
% cellSL_binary_edges = edge(cellSL_binary_cleaned,'log','thinning');
% % imshow(cellSL_binary)
% 
% %***ColeD change starts: 
% % Added final 'bwareaopen' to clean edges
% % This last line removes any straggling edges as they will be far smaller
% % in length than our image border
% cellSL_binary_final = bwareaopen(cellSL_binary_edges,100);
% % imshow(cellSL_binary_final)
% %***ColeD change ends
% 
% %***ColeD_long changes end

%***ColeD change starts:
% I want to compare the original image edge with the newly defined one
%
% Code starts here:
% %
% % Display the original edge detection (red channel) and the final cleaned-up edge detection (green channel) overlaid
% imshow(cat(3, cell_img, cellSL_binary_cleaned, zeros(size(cellSL_binary_cleaned))));
% title('Original Edges (Red) vs. Cleaned Edges (Green)');
%***ColeD change ends
...
%[cell_edgelist,cell_edgeim] = edgelink(cellSL_binary_final);
...
%disp("T-Tubule PLC creation is currently commented out for debugging")
disp("Creating T-tubule PLC")
% 

%***Cole D changes start:
% be sure to comment out this section, it is purely for debugging without
% stepping into the function
% % 
% % sed = strel('disk',4);
% % sel = strel('line', 8, 45);
% % test = bwareaopen(tt_bin,5);
% % test = imdilate(test,sed);
% % %imshow(test)
% % test = imopen(test,sed);
% % test = imerode(test,sel);
% % test = bwareaopen(test,5);
% % 
% % alpha = .7;
% % cyan = [0,0,0];
% % overlay = RGB;
% % 
% % % Red channel
% % overlay(:,:,1) = overlay(:,:,1) .* ~test + ...
% %     ((1 - alpha) * overlay(:,:,1) + alpha * cyan(1)) .* test;
% % % Green channel
% % overlay(:,:,2) = overlay(:,:,2) .* ~test + ...
% %     ((1 - alpha) * overlay(:,:,2) + alpha * cyan(2)) .* test;
% % % Blue channel
% % overlay(:,:,3) = overlay(:,:,3) .* ~test + ...
% %     ((1 - alpha) * overlay(:,:,3) + alpha * cyan(3)) .* test;
% % % figure;
% % % imshow(overlay);

%***Cole D debug changes end
...
%disp("mitochondria PLC creation is currently commented out for debugging")
disp("Creating mitochondria PLC")

%***Cole D changes start:
% be sure to comment out this section, it was purely for debugging without
% stepping into the function

% % % sed = strel('disk',4);
% % % sel = strel('line', 8, 45);
% % % test = bwareaopen(mito_bin,5);
% % % test = imdilate(test,sed);
% % % %imshow(test)
% % % test = imopen(test,sed);
% % % test = imerode(test,sel);
...
%***Cole D changes start:
% be sure to remove this section, it was purely for debugging without
% stepping into the function
...
% % figure;
% % imshow(glyco_bin);
% test = bwareaopen(glyco_bin,5);
% % figure;
% % imshow(test);
% % test = imdilate(test,sed);
% test = imdilate(test,sed);
% test = imdilate(test,sel);
% %
% % figure;
% % imshow(test)
% % figure;
...
for k = 1:total_sl_nodes
    fprintf (fid,strcat(num2str(k),'\t',num2str(all_nodes(k,2)),'\t',num2str(all_nodes(k,1)),'\t',num2str(0),'\t',num2str(1),'\n'));
end
%
for k = (total_sl_nodes+1):(total_sl_nodes+total_TT_edges)
    fprintf (fid,strcat(num2str(k),'\t',num2str(all_nodes(k,2)),'\t',num2str(all_nodes(k,1)),'\t',num2str(3),'\t',num2str(5),'\n'));
end
