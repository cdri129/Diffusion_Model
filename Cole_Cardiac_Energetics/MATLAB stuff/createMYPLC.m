...
% se1 = strel('disk', 7);
% se2 = strel('disk', 2);
% myo_bin_after = imopen(myo_bin, se1); % Apply morphological opening
% myo_bin_after = imdilate(~myo_bin_after,se2);
% myo_bin_after = ~myo_bin_after;
...
% 
% figure;
% imshow(myo_bin_after);
% 
%***ColeD changes end
%7
se1 = strel('disk', 8);
se2 = strel('disk', 4);
myo_bin = imopen(myo_bin, se1);
myo_bin = imdilate(~myo_bin,se2);
myo_bin = ~myo_bin;
...
for label = 1:MY_nums
    ...
    MY_boundary = bwareaopen (MY_boundary, 8);


    % hold on;
    % h = imshow(cat(3, MY_boundary, zeros(size(MY_boundary)), zeros(size(MY_boundary))), 'InitialMagnification', 'fit'); % Display before image in red channel
    % set(h, 'AlphaData', 0.5); % Set transparency for before image
    % h2 = imshow(cat(3, zeros(size(MY_boundary_f)), MY_boundary_f, zeros(size(MY_boundary_f))), 'InitialMagnification', 'fit'); % Display after image in green channel
    % set(h2, 'AlphaData', 0.5); % Set transparency for after image
    % hold off;
    %***ColeD changes start:
